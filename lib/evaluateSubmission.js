...
const { prepareLuaSourceForEvaluation } = require('./prepareLuaForEval')

/** Subtracted from worker `memory_used` before persisting as `memory_bytes`. */
const POSTED_MEMORY_BYTES_OFFSET = 2652

/** Starting gas budget; posted `gas` is this minus worker `gas_remaining` (gas used). */
const GAS_BASELINE = 1_000_000
...
function metricsFromBody(body) {
  ...
  const out = {}
  if (typeof body.gas_remaining === 'number') {
    out.gas = Math.max(0, GAS_BASELINE - body.gas_remaining)
  }
  if (typeof body.memory_used === 'number') {
    out.memory_bytes = Math.max(0, body.memory_used - POSTED_MEMORY_BYTES_OFFSET)
  }
  return out
}
...
async function evaluateSubmission({ userSource, challenge }) {
  ...
  try {
    ...
    const source = prepareLuaSourceForEvaluation(userSource)
    let printOutput = []
    {
